      ...
      {feed?.map((res: FeedType, index: number) => {
        ...
        return (
          <CardList size={checkSize} key={res.id}>
            <Card aspect={checkAspect} to={"/detail"} state={res.id}>
              <WeatherEmojiBox>
                ...
                <UserReactBox>
                  <UserIconBox>
                    <UserIcon onClick={() => toggleLike(res)}>
                      {userObj?.like?.filter((id) => id === res.id).length >
                      0 ? (
                  ...
                  <UserIcon onClick={() => toggleBookmark(res.id)}>
                    {userObj?.bookmark?.filter((id) => id === res.id).length >
                    0 ? (
